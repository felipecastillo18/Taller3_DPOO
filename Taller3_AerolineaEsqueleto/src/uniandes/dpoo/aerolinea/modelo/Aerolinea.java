...
public class Aerolinea {
	...
	public Vuelo getVuelo(String codigoRuta, String fechaVuelo) {
		...
		for (Vuelo vuelo : this.vuelos) {
			boolean mismaFecha = fechaVuelo.equals(vuelo.getFecha());

			boolean mismaRuta = codigoRuta.equals(vuelo.getRuta().getCodigoRuta());

			if (mismaFecha && mismaRuta) {
				return vuelo;
			}
		}
		return null;
	...
	public void programarVuelo(String fecha, String codigoRuta, String nombreAvion) throws Exception {
		...
		if (ruta == null)
			throw new Exception("La ruta " + codigoRuta + " no existe");
		...
		if (getVuelo(codigoRuta, fecha) != null)
			throw new Exception("Ya existe un vuelo de la ruta " + codigoRuta + " el " + fecha);

		// Verificar que el avión no esté ocupado en otro vuelo el mismo día en un
		// horario que se cruce
		int salida = Ruta.getHoras(ruta.getHoraSalida()) * 60 + Ruta.getMinutos(ruta.getHoraSalida());
		int llegada = salida + ruta.getDuracion();
		for (Vuelo otro : vuelos) {
			if (otro.getAvion() == avion && otro.getFecha().equals(fecha)) {
				Ruta otraRuta = otro.getRuta();
				int otraSalida = Ruta.getHoras(otraRuta.getHoraSalida()) * 60
						+ Ruta.getMinutos(otraRuta.getHoraSalida());
				int otraLlegada = otraSalida + otraRuta.getDuracion();
				if (salida < otraLlegada && otraSalida < llegada)
					throw new Exception("El avión " + nombreAvion + " ya está ocupado en ese horario");
			}
		}
	...
	public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad)
		...
		if (cliente == null)
			throw new Exception("El cliente " + identificadorCliente + " no existe");

		Vuelo vuelo = getVuelo(codigoRuta, fecha);
		if (vuelo == null)
			throw new Exception("No existe un vuelo de la ruta " + codigoRuta + " el " + fecha);

		// La fecha tiene el formato aaaa-mm-dd
